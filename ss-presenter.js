(function () {
  ...
  function blankGame() {
    return {
      ...
      secs: 60, totalRounds: 8,
      packs: MLT.SS_PACKS.map(function (p) { return p.id; }),
      players: [], order: [], N: 0,
      round: 0, artistId: null, word: null, usedWords: [],
      liveStrokes: [], guessed: {}, guessOrder: [],
      endsAt: null, history: [],
  ...
  if (!G.packs || !G.packs.length) G.packs = MLT.SS_PACKS.map(function (p) { return p.id; });
  if (!G.order) G.order = [];
  if (!G.usedWords) G.usedWords = [];
  if (!G.liveStrokes) G.liveStrokes = [];
  ...
  function enterRoom() {
    ...
    renderAll();
    makeQR();
    if (G.phase === 'draw') { runClock(); sendSecretWord(); }
    broadcast(true);
    A.ready();
    A.music(G.phase === 'draw' ? 'round' : 'lobby');
  }
  ...
  function poolFromPacks() {
    var pool = [];
    MLT.SS_PACKS.forEach(function (pk) {
  ...
  function pickWord() {
    ...
    if (!pool.length) return 'mystery';
    var avail = pool.filter(function (w) { return G.usedWords.indexOf(w) < 0; });
    if (!avail.length) { G.usedWords = []; avail = pool.slice(); }
  ...
  function publicState() {
    ...
    if (G.phase === 'draw') {
      ...
      s.artistId = G.artistId;
      s.secs = G.secs; s.endsAt = G.endsAt;
      s.strokes = G.liveStrokes;
      ...
      s.artistPts = last ? last.artistPts : 0;
      s.guessed = G.guessed;
  ...
  function sendSecretWord() {
    if (bus && G.artistId) bus.send({ t: 'secret', to: G.artistId, round: G.round, word: G.word });
  }
  ...
  function onBusMessage(m) {
    if (m.t === 'hello') {
      ...
      else if (d === 'skip') { if (G.phase === 'draw') doReveal(); }
      else if (d === 'timer+') { if (G.phase === 'draw') adjustTimer(5); }
      else if (d === 'timer-') { if (G.phase === 'draw') adjustTimer(-5); }
      else if (d === 'timerOff') { if (G.phase === 'draw') toggleNoTimer(); }
      else if (d === 'next') { if (G.phase === 'reveal') nextRound(); }
      else if (d === 'again') { if (G.phase === 'final') playAgain(); }

    } else if (m.t === 'strokes') {
      if (G.phase !== 'draw' || m.from !== G.artistId || m.round !== G.round) return;
      G.liveStrokes = MLT.ssSanitizeStrokes(m.strokes);
      save(); broadcast(false);

    } else if (m.t === 'needword') {
      if (G.phase === 'draw' && m.from === G.artistId) sendSecretWord();

    } else if (m.t === 'guess') {
      if (G.phase !== 'draw' || m.round !== G.round) return;
      if (m.from === G.artistId) return;
      if (G.guessed[m.from] !== undefined) return;
      if (!playerById(m.from)) return;
      var text = String(m.text || '').slice(0, 60);
      if (MLT.ssNormalize(text) === MLT.ssNormalize(G.word) && MLT.ssNormalize(text)) {
  ...
  function startRound() {
    G.phase = 'draw';
    G.artistId = G.order[G.round % G.N];
    G.word = pickWord();
    G.liveStrokes = [];
    G.guessed = {};
    G.guessOrder = [];
  ...
  function adjustTimer(delta) {
    var wasOff = !(G.secs > 0);
    G.secs = Math.max(15, Math.min(180, (G.secs > 0 ? G.secs : 0) + delta));
    if (G.phase === 'draw') {
  ...
  function toggleNoTimer() {
    if (G.secs > 0) {
      ...
      if (G.phase === 'draw') G.endsAt = Date.now() + G.secs * 1000;
    }
    if (G.phase === 'draw') runClock();
    save(); renderDraw(); broadcast(true);
  }
  ...
  function doReveal() {
    ...
    var artistPts = totalGuessers > 0 ? Math.round(100 * correctCount / totalGuessers) : 0;
    var artistP = playerById(G.artistId);
    if (artistP) artistP.pts += artistPts;
    G.history.push({
      word: G.word, artistId: G.artistId, correctCount: correctCount,
      totalGuessers: totalGuessers, artistPts: artistPts,
      guessed: Object.assign({}, G.guessed)
    });
    save();
    show('reveal');
    renderReveal();
  ...
  function playAgain() {
    G.phase = 'lobby';
    G.round = 0; G.artistId = null; G.word = null; G.usedWords = [];
    G.liveStrokes = []; G.guessed = {}; G.guessOrder = []; G.endsAt = null;
    G.history = [];
    G.players.forEach(function (p) { p.pts = 0; });
    save();
  ...
  function computeAwards() {
    ...
    var drawTotals = {}, guessTotals = {};
    G.history.forEach(function (h) {
      drawTotals[h.artistId] = (drawTotals[h.artistId] || 0) + h.artistPts;
    ...
    var awards = [];
    var bestArtist = null;
    Object.keys(drawTotals).forEach(function (id) {
      if (!playerById(id)) return;
      if (!bestArtist || drawTotals[id] > drawTotals[bestArtist]) bestArtist = id;
    });
    if (bestArtist && drawTotals[bestArtist] > 0) {
      awards.push({ t: 'Picasso', p: bestArtist, c: '#9d6bff', d: 'Got the most guessers correct across every drawing this game' });
    }
    var fastest = null;
    Object.keys(guessTotals).forEach(function (id) {
      if (!playerById(id)) return;
      if (!fastest || guessTotals[id] > guessTotals[fastest]) fastest = id;
    });
    if (fastest && guessTotals[fastest] > 0) {
      awards.push({ t: 'Fastest Fingers', p: fastest, c: '#3dff9e', d: 'Racked up the most points guessing quickly and correctly' });
    }
    return awards;
  }
  ...
  function renderLobby() {
    ...
    $('#lobbyHint').textContent = G.players.length < 3
      ? 'Waiting for people to join… you need at least 3 — one to draw, at least two to guess.'
      : avail < 1
        ? 'Pick at least one word pack in Settings.'
  ...
  function renderPacks() {
    ...
    if (!box) return;
    box.innerHTML = '';
    MLT.SS_PACKS.forEach(function (p) {
    ...
    $('#qcount').textContent = avail
      ? avail + ' word' + (avail === 1 ? '' : 's') + ' ready' + (G.packs.length ? ' · ' + G.packs.length + ' pack' + (G.packs.length > 1 ? 's' : '') : '')
      : 'Pick at least one pack.';
  }

  function renderDraw() {
    document.documentElement.style.setProperty('--accent', ACCENT);
    $('#dProgress').textContent = 'Round ' + (G.round + 1) + ' / ' + G.totalRounds;
    $('#dArtist').textContent = nameOf(G.artistId) + ' is drawing';
    $('#dCanvasView').innerHTML = MLT.ssStrokesToSVG(G.liveStrokes, MLT.SS_CANVAS_W, MLT.SS_CANVAS_H);
    var correct = Object.keys(G.guessed).length;
    var total = Math.max(0, G.N - 1);
    $('#dCorrectCount').textContent = correct;
    $('#dCorrectTotal').textContent = total;
    $('#secsNow').textContent = G.secs > 0 ? G.secs + 's per round' : 'no countdown';
    $('#btnNoTimer').textContent = G.secs > 0 ? 'No timer' : 'Timer on';
    $('#btnMinus').disabled = !(G.secs > 15);
  ...
  function renderReveal() {
    ...
    $('#rWord').textContent = G.word;
    $('#rCanvasView').innerHTML = MLT.ssStrokesToSVG(G.liveStrokes, MLT.SS_CANVAS_W, MLT.SS_CANVAS_H);
    $('#rArtistName').textContent = nameOf(G.artistId);
    $('#rArtistPts').textContent = '+' + (last.artistPts || 0) + ' pts';
    ...
    var missed = G.players.filter(function (p) { return p.id !== G.artistId && G.guessed[p.id] === undefined; });
    if (missed.length) {
  ...
  function renderFinal() {
    ...
    $('#finalTitle').textContent = sorted.length ? sorted[0].name + ' draws (and guesses) the best' : 'Final standings';
    MLT.confetti(sorted.slice(0, 3).map(function (p) { return MLT.colorFor(indexOfPlayer(p.id)); }));
  }

  function renderAll() {
    if (G.phase === 'lobby') renderLobby();
    else if (G.phase === 'draw') renderDraw();
  ...
  $('#packAll').addEventListener('click', function () {
    G.packs = MLT.SS_PACKS.map(function (p) { return p.id; });
    save(); renderPacks(); renderLobby();
  });
  ...
  if (wantNew) newRoom();
  else if (saved && saved.phase === 'draw') enterRoom();
  else openStart();
